...
public class Playback extends JPanel implements ActionListener, ComponentListener {
...
// Open up the container
        if (container.open(whatToPlay, IContainer.Type.READ, null) < 0) {
            throw new IllegalArgumentException(
                    "could not open file: " + whatToPlay
            );
        }
...
// Get the pre-configured decoder that can decode this stream;
        ...
        if (videoStreamId == -1) {
            throw new RuntimeException(
                    "could not find video stream in container: " + whatToPlay
            );
        }
...
* do work.
        ...
        if (videoCoder.open() < 0) {
            throw new RuntimeException(
                    "could not open video decoder for container " + whatToPlay
            );
        }

        IVideoResampler resampler = null;

        if (videoCoder.getPixelType()
...
// just return from the method.
    ...
    public int jumpToTime(long time) {
        System.out.println(1000 * time);
        return jumpToFrame(1000*time );
    }

    public int jumpToFrame(long framecount) {
        int result = -1;
        try {
            if (container != null) {
                result = container.seekKeyFrame(-1, framecount, IContainer.SEEK_FLAG_BACKWARDS);
            }
        } catch (Exception e) {
            isClosed = true;
            e.printStackTrace();
...
//container.seekKeyFrame(0,0,framecount,container.getDuration(),IContainer.SEEK_FLAG_ANY);
    ...
    public static void main(String[] args) {
        JFrame j = new JFrame();
        j.setSize(500, 400);
        Playback p = new Playback("E:\\Dropbox\\", "Intro1.mp4");
